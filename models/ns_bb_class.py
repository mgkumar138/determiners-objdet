...
if trainwithcaptions:
    history = model.fit(x=[tr_bb_rs,tr_emb],y=[tr_out_rs, tr_cls_id],
                        validation_data=([val_bb_rs,val_emb],[val_out_rs, val_cls_id]),
...
# train data
tr_out[:,:,:4] *= 256.0
[pred_tr_score, pred_tr_cls] = model.predict([tr_bb_rs,tr_emb])
...
# create_output_txt(gdt=tr_out, predt=pred_tr_bbcap, confi=pred_tr_score,gd_cls=tr_cls_id,pred_cls=pred_tr_cls,directory='ns_cls/train_bb_cap')
#
# # train data
val_out[:,:,:4] *= 256.0
[pred_val_score, pred_val_cls] = model.predict([val_bb_rs,val_emb])
...
# create_output_txt(gdt=val_out, predt=pred_val_bbcap, confi=pred_val_score,gd_cls=val_cls_id,pred_cls=pred_val_cls, directory='ns_cls/val_bb_cap')

# test with both inputs
ts_out[:,:,:4] *= 256.0
[pred_ts_score, pred_ts_cls] = model.predict([ts_bb_rs,ts_emb])
...
#create_output_txt(gdt=ts_out, predt=pred_ts_bbcap, confi=pred_ts_score,gd_cls=ts_cls_id,pred_cls=pred_ts_cls, directory='ns_cls/test_bb_cap')

# test with bb but no captions
[pred_ts_score_nocap, pred_ts_cls_nocap] = model.predict([ts_bb_rs,np.zeros_like(ts_emb)])
...
#create_output_txt(gdt=ts_out, predt=pred_ts_bb, confi=pred_ts_score_nocap,gd_cls=ts_cls_id,pred_cls=pred_ts_cls_nocap, directory='ns_cls/test_bb_only')
